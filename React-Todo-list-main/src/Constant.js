export const APP_TITLE = "Sample App";
export const FEATURE_GATE_1 = "sample_feature_gate";
export const DYNAMIC_CONFIG_1 = "sample_dynamic_config";
export const EXPERIMENT_1 = "sample_experiment";
export const LAYER_1 = "sample_layer";
export const TODO_EDITED = "TODO_EDITED";
export const TODO_COMPLETED = "TODO_COMPLETED";
export const TODO_CREATED = "TODO_CREATED";
export const TODO_DELETED = "TODO_DELETED";
export const TODO_LAST_VIEWED = "TODO_LAST_VIEWED";
export const EXPERIMENT_SORTING = "item_sorting";
export const SS_CLIENT_SDK_KEY = "client-wWQtJlfJfEnLSIuP6QcnTATHyGVHUnJ2URTGLOVrwrl";
export const DYNAMIC_CONFIG_WARNING_BANNER = "warning_banner"
export const SS_TODO_APP_CLIENT_KEY = "client-iVVxwevG2GuTIRErepyVB3ZWYmLNwHSABBVwWta7uYF";


